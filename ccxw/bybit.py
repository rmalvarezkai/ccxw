...
class BybitAuxClass():
    ...
    def if_symbol_supported(self):
        ...
        if isinstance(__data,dict) and 'retCode' in __data and __data['retCode'] == 0\
            and 'result' in __data and isinstance(__data['result'],dict)\
            and 'list' in __data['result'] and isinstance(__data['result']['list'],list)\
            and len(__data['result']['list']) > 0 and isinstance(__data['result']['list'][0],dict):
            if 'symbol' in __data['result']['list'][0]\
                and isinstance(__data['result']['list'][0]['symbol'], str)\
                and str(self.__ws_symbol.replace('/','').upper())\
                    == __data['result']['list'][0]['symbol']\
                and isinstance(__data['result']['list'][0]['status'], str)\
                and 'trading' == __data['result']['list'][0]['status'].lower():
                result = True
        ...
        try:
            ...
            if ccf.is_json(message_in):
                ...
                if self.__ws_endpoint == 'order_book':
                    ...
                    if __temp_data is not None and isinstance(__temp_data,dict)\
                        and 'ts' in __temp_data and 'data' in __temp_data\
                        and isinstance(__temp_data['data'],list) and len(__temp_data['data']) > 0:
                        ...
                        for i in range(len(__temp_data['data']) - 1,-1,-1):
                            ...
                            __message_add['trade_id'] = __temp_data['data'][i]['i']
                            __message_add['price'] = __temp_data['data'][i]['p']
                            __message_add['quantity'] = __temp_data['data'][i]['v']
                    ...
                    if __temp_data is not None and isinstance(__temp_data,dict)\
                        and 'cs' in __temp_data and 'data' in __temp_data\
    ...
    def __manage_websocket_diff_data(self,diff_data):
        ...
        if isinstance(diff_data,dict) and 'ts' in diff_data and 'data' in diff_data\
            ...
            and 'b' in diff_data['data'] and 'a' in diff_data['data']\
            ...
            for i in range(0,len(current_data['data'][__key])):
                __temp_bids_d[current_data['data'][__key][i][0]] = current_data['data'][__key][i][1]
            ...
            for i in range(0,len(diff_data['data'][__key])):
                if float(diff_data['data'][__key][i][1]) == 0:
                    ...
                    __temp_bids_d[diff_data['data'][__key][i][0]] = diff_data['data'][__key][i][1]
            ...
            __key = 'a'
            for i in range(0,len(current_data['data'][__key])):
                __temp_asks_d[current_data['data'][__key][i][0]] = current_data['data'][__key][i][1]

            for i in range(0,len(diff_data['data'][__key])):
                if float(diff_data['data'][__key][i][1]) == 0:
                    n_t = __temp_asks_d.pop(diff_data['data'][__key][i][0],None)
                else:
                    __temp_asks_d[diff_data['data'][__key][i][0]] = diff_data['data'][__key][i][1]
