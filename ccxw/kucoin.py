...
class KucoinAuxClass():
    ...
    def if_symbol_supported(self):
        ...
        if isinstance(__data,dict) and 'code' in __data and __data['code'] == '200000'\
            and 'data' in __data and isinstance(__data['data'],list) and len(__data['data']) > 0:
            __res_data = (
                [d for d in __data['data'] if d['symbol'] ==\
                 self.__ws_symbol.replace("/","-").upper()]
            )
            if isinstance(__res_data,list) and len(__res_data) > 0\
                and isinstance(__res_data[0],dict)\
                and 'enableTrading' in __res_data[0] and __res_data[0]['enableTrading']:
                result = True
            ...
            if ccf.is_json(message_in):
                ...
                if self.__ws_endpoint == 'order_book':
                    ...
                    if __temp_data is not None and isinstance(__temp_data,dict)\
                        and 'type' in __temp_data and 'data' in __temp_data\
                        and isinstance(__temp_data['data'],dict)\
                        and 'candles' in __temp_data['data']\
                    ...
                    if __temp_data is not None and isinstance(__temp_data,dict)\
                        and 'type' in __temp_data and __temp_data['type'] == 'message'\
                        ...
                        __message_add['trade_id'] = __temp_data['data']['tradeId']
                        __message_add['price'] = __temp_data['data']['price']
                        __message_add['quantity'] = __temp_data['data']['size']
